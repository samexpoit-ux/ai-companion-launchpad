  ...
  return (
    ...
    >
      ...
      <div ref={switcherRef} className="relative px-3">
        ...
        >
          <span className="grid h-6 w-6 shrink-0 place-items-center rounded-md text-[11px] font-bold text-[color:var(--color-iris-fg)] iris-bg">
          ...
          <div className="absolute left-3 right-3 top-[calc(100%+6px)] z-40 overflow-hidden rounded-xl border border-ink-200 bg-ink-50 shadow-ds-lg">
            <div className="border-b border-ink-200 p-3">
              <div className="flex items-center gap-2.5">
                <span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg text-[13px] font-bold text-[color:var(--color-iris-fg)] iris-bg">
                  {workspaceName.slice(0, 1).toUpperCase()}
                </span>
                <div className="min-w-0">
                  <p className="truncate text-[13px] font-semibold text-ink-900">{workspaceName}</p>
                  <p className="text-[11px] text-ink-500">Free plan · 1 member</p>
                </div>
              </div>
            ...
            <div className="border-b border-ink-200 p-3">
              <div className="flex items-center justify-between">
                <span className="text-[12px] font-medium text-ink-700">Credits</span>
                <span className="text-[11px] font-medium text-[color:var(--color-iris-ink)]">
                  {credits.left} left
                </span>
              </div>
            ...
            <div className="p-2">
              <p className="px-1.5 pb-1 text-[11px] font-medium text-ink-400">Workspaces</p>
              <button
                type="button"
                className="flex w-full items-center gap-2 rounded-md px-1.5 py-1.5 text-left transition hover:bg-ink-200/60"
              >
                <span className="grid h-6 w-6 place-items-center rounded-md bg-ink-800 text-[10px] font-bold text-ink-50">
                  {userLabel.slice(0, 1).toUpperCase()}
                </span>
                <span className="min-w-0 flex-1 truncate text-[12.5px] text-ink-700">{userLabel}</span>
                <span className="rounded bg-ink-200 px-1.5 py-0.5 text-[10px] text-ink-500">Free</span>
              </button>
              <button
                type="button"
                className="mt-1 flex w-full items-center gap-2 rounded-md px-1.5 py-1.5 text-[12.5px] text-ink-600 transition hover:bg-ink-200/60"
              >
            ...
            <div className="border-t border-ink-200 p-2">
              <button
                type="button"
                className="flex w-full items-center gap-2 rounded-md px-1.5 py-1.5 text-[12.5px] text-ink-600 transition hover:bg-ink-200/60"
              >
              ...
              <div className="mt-1 flex items-center justify-between rounded-md px-1.5 py-1.5">
                ...
                <span className="rounded bg-[color:var(--color-iris-soft)] px-1.5 py-0.5 text-[10px] font-medium text-[color:var(--color-iris-ink)]">
                  Upgrade
                </span>
              </div>
            </div>
        ...
        {NAV.map((item) => {
          ...
          return (
            ...
            >
              ...
              {item.hint && (
                <kbd className="rounded border border-ink-200 bg-ink-50 px-1 text-[10px] text-ink-400">
                  {item.hint}
                </kbd>
              )}
            </Link>
      ...
      <div className="mt-5 px-3">
        <p className="px-2 pb-1 text-[11px] font-medium text-ink-400">Projects</p>
        {PROJECT_NAV.map((item) => (
          <button
            key={item.label}
            type="button"
            className="flex w-full items-center gap-2.5 rounded-lg px-2 py-1.5 text-left text-[13px] text-ink-600 transition hover:bg-ink-200/60 hover:text-ink-900"
          >
            <item.icon className="h-4 w-4 shrink-0" />
            <span className="min-w-0 flex-1 truncate">{item.label}</span>
          </button>
      ...
      <div className="mt-5 min-h-0 flex-1 overflow-y-auto px-3 pb-3">
        ...
        {recents.length === 0 ? (
          <p className="px-2 py-1 text-[12px] text-ink-400">No projects yet</p>
        ) : (
      ...
      <div className="border-t border-ink-200 p-3">
        <div className="flex items-center justify-between rounded-xl border border-ink-200 bg-ink-50 px-3 py-2.5 shadow-ds-xs">
          <div className="min-w-0">
            <p className="text-[12.5px] font-semibold text-ink-900">Share Nexus X</p>
            <p className="truncate text-[11px] text-ink-400">100 credits per paid referral</p>
          </div>
          <Gift className="h-4 w-4 shrink-0 text-[color:var(--color-iris)]" />
        </div>

        <div className="mt-2 flex items-center gap-2">
          <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-ink-800 text-[11px] font-semibold text-ink-50">
            {userLabel.slice(0, 1).toUpperCase()}
          </span>
          <span className="min-w-0 flex-1 truncate text-[12.5px] text-ink-600">{userLabel}</span>
          <button
            type="button"
            aria-label="Sign out"
