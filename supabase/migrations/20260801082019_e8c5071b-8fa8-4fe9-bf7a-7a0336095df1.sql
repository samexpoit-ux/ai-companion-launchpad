...
BEGIN
  ...
  WHERE user_id = target AND credits > 0
  ...
  IF settings_row.period_start IS NULL
     OR settings_row.period_start < date_trunc('month', now()) THEN
  ...
  remaining := settings_row.credits_total - spent;
  IF remaining < _credits THEN
    RAISE EXCEPTION 'insufficient credits: % remaining', GREATEST(remaining, 0);
  END IF;

  INSERT INTO public.credit_ledger
    (user_id, action, tier, credits, model, thread_id, reason)
  VALUES
    (target, _action, _tier, _credits, _model, _thread_id, _reason)
...
REVOKE ALL ON FUNCTION public.spend_credits(TEXT, TEXT, NUMERIC, TEXT, UUID, TEXT, UUID)
  FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.spend_credits(TEXT, TEXT, NUMERIC, TEXT, UUID, TEXT, UUID)
